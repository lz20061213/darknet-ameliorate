...
box get_mutual_yolo_box(float *x, float *biases, int n, int index, int i, int j, int lw, int lh, int w, int h, int stride) {
    box b;
...
float delta_mutual_yolo_box(box truth, float *x, float *mutual_x, float *biases, int n, int index, int i, int j, int lw,
                     int lh, int w, int h, float *delta, float scale, int stride, float margin) {
    box pred = get_mutual_yolo_box(x, biases, n, index, i, j, lw, lh, w, h, stride);
    box mutual_pred = get_mutual_yolo_box(mutual_x, biases, n, index, i, j, lw, lh, w, h, stride);
    float iou = box_iou(pred, truth);
    float mutual_iou = box_iou(mutual_pred, truth);
    ...
    if (iou + margin * 2 < mutual_iou)  scale *= 1.5;
    ...
    for (b = 0; b < ls.batch; ++b) {
        for (j = 0; j < ls.h; ++j) {
            for (i = 0; i < ls.w; ++i) {
                for (n = 0; n < ls.n; ++n) {
                    ...
                    box spred = get_mutual_yolo_box(ls.output, ls.biases, ls.mask[n], box_index, i, j, ls.w, ls.h, snet.w, snet.h,
                                              ls.w * ls.h);
                    ...
                    box ppred = get_mutual_yolo_box(lp.output, lp.biases, lp.mask[n], box_index, i, j, lp.w, lp.h, pnet.w, pnet.h,
                                              lp.w * lp.h);
                    ...
                    if (sbest_iou > ls.truth_thresh) {
                        ...
                        delta_mutual_yolo_box(struth, ls.output, lp.output, ls.biases, ls.mask[n], box_index, i, j,
                    ...
                    if (pbest_iou > lp.truth_thresh) {
                        ...
                        delta_mutual_yolo_box(ptruth, lp.output, ls.output, lp.biases, lp.mask[n], box_index, i, j,
        ...
        for (t = 0; t < ls.max_boxes; ++t) {
            ...
            if (mask_n >= 0) {
                ...
                float siou = delta_mutual_yolo_box(truth, ls.output, lp.output, ls.biases, best_n, box_index, i, j,
                                        ls.w, ls.h, snet.w, snet.h, ls.delta, (2 - truth.w * truth.h), ls.w * ls.h, ls.margin);
                float piou = delta_mutual_yolo_box(truth, lp.output, ls.output, lp.biases, best_n, box_index, i, j,
