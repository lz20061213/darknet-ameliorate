...
box_label *correct_boxes_with_thresh(box_label *boxes, int *n, float dx, float dy, float sx, float sy, int flip, float thresh) {
    int i;
    ...
    for(i = 0; i < *n; ++i){
        ...
        float ori_area = (boxes[i].bottom - boxes[i].top) * (boxes[i].right - boxes[i].left);
        ...
        float cor_area = boxes[i].w * boxes[i].h;
    ...
    // get new boxes
    box_label *filter_boxes = calloc(count, sizeof(box_label));
    int j = 0;
    for(i = 0; i < *n; ++i) {
        if (out_of_thresh[i]) continue;
        filter_boxes[j].id = boxes[i].id;
        filter_boxes[j].x = boxes[i].x;
        filter_boxes[j].y = boxes[i].y;
        filter_boxes[j].w = boxes[i].w;
        filter_boxes[j].h = boxes[i].h;
        filter_boxes[j].left= boxes[i].left;
        filter_boxes[j].right = boxes[i].right;
        filter_boxes[j].top = boxes[i].top;
        filter_boxes[j].bottom = boxes[i].bottom;
        j++;
    }
...
void fill_truth_swag(char *path, float *truth, int classes, int flip, float dx, float dy, float sx, float sy)
{
    ...
    correct_boxes(boxes, count, dx, dy, sx, sy, flip);   // check boxes
    float x,y,w,h;
    int id;
    int i;
...
void fill_truth_detection(char *path, int num_boxes, float *truth, int classes, int flip, float dx, float dy, float sx, float sy, float filter_thresh)
{
    char labelpath[4096];
    find_replace(path, "images", "labels", labelpath);
    ...
    if (filter_thresh > 0.01) {
        boxes = correct_boxes_with_thresh(boxes, &count, dx, dy, sx, sy, flip, filter_thresh);
    } else {
        correct_boxes(boxes, count, dx, dy, sx, sy, flip);
    }
...
data single_data_transform(char* path, int w, int h, int boxes, int classes, float jitter, float hue, float saturation, float exposure, int openscale,
    float filter_thresh, float min_scale, float max_scale)
{
    ...
    d.y = make_matrix(d.X.rows, 5*boxes);
    ...
    fill_truth_detection(path, boxes, d.y.vals[0], classes, flip, -dx/w, -dy/h, nw/w, nh/h, filter_thresh);
    ...
    int i;
    for (i = 0; i < boxes; ++i) {
        float x = truth[i * 5 + 0] * sized.w;
        float y = truth[i * 5 + 1] * sized.h;
        float w = truth[i * 5 + 2] * sized.w;
        float h = truth[i * 5 + 3] * sized.h;
