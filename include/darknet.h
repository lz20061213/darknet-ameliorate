...
typedef enum{
    SSE, MASKED, L1, SEG, SMOOTH, WGAN, L1_MARGIN
} COST_TYPE;
...
struct layer{
    ...
    void (*mutual_forward)(struct layer, struct network, struct network);
    void (*mimicutual_forward)(struct layer, struct network, struct network, struct network);
    void (*backward)  (struct layer, struct network);
    void (*update)    (struct layer, update_args);
    void (*forward_gpu)   (struct layer, struct network);
    void (*mimic_forward_gpu)(struct layer, struct network, struct network);
    void (*mutual_forward_gpu)(struct layer, struct network, struct network);
    void (*mimicutual_forward_gpu)(struct layer, struct network, struct network, struct network);
    ...
    float lb_dis_max_thresh;
    float lb_dis_ignore_thresh;
    float lb_dis_truth_thresh;
...
typedef struct network{
    ...
    int random;
    int openscale;
    // data_fusion_type: mixup, cutmix, mosaic, mosaic+cutmix, mosaic+mixup
    int data_fusion_type;
    // data_fusion_prob, for each image, has prob to do data_fusion
...
void scal_add_gpu(int N, float ALPHA, float BATE, float * X, int INCX);
void reweight_gpu(int N, float * scale, int scale_offset, float * X, int INCX, float * Y, int INCY);
